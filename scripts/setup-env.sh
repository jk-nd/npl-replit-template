...
# This script derives all URLs from NPL_TENANT and NPL_APP_NAME
...
if [ -z "$NPL_APP_NAME" ]; then
    echo "❌ Error: NPL_APP_NAME environment variable not set"
...
ENGINE_URL="https://engine-${NPL_TENANT}-${NPL_APP_NAME}.noumena.cloud"
KEYCLOAK_URL="https://keycloak-${NPL_TENANT}-${NPL_APP_NAME}.noumena.cloud"
KEYCLOAK_REALM="${NPL_APP_NAME}"
KEYCLOAK_CLIENT_ID="${NPL_APP_NAME}"

echo ""
echo "📋 Derived configuration:"
echo "   NPL_TENANT:            $NPL_TENANT"
echo "   NPL_APP_NAME:          $NPL_APP_NAME"
...
cat > .env << EOF
# Auto-generated from NPL_TENANT and NPL_APP_NAME
...
# Set to 'true' to bypass Keycloak for local UI development
VITE_DEV_MODE=false
EOF
...
echo ""
echo "🔗 Useful links:"
echo "   Portal:    https://portal.noumena.cloud/${NPL_TENANT}/${NPL_APP_NAME}"
